...
int vex_init()
{
    ...
    vexc_debug("Initializing VEX.\n");

    if (initialized)
    {
        vexc_debug("VEX already initialized.\n");
        return 1;
    }
    initialized = 1;
    ...
    vexc_debug("Calling LibVEX_Init()....\n");
    if (setjmp(jumpout) == 0)
    {
        // the 0 is the debug level
        LibVEX_Init(&failure_exit, &log_bytes, 0, &vc);
        vexc_debug("LibVEX_Init() done....\n");
    }
    else
    {
        vexc_debug("LibVEX_Init() failed catastrophically...\n");
        return 0;
    }
...
static void vex_prepare_vai(VexArch arch, VexArchInfo *vai)
{
    ...
    case VexArchRISCV64:
        ...
        break;
    default:
        vexc_error("Invalid arch in vex_prepare_vai.\n");
        break;
    }
}
    ...
    unsigned int lookback)
{
    ...
    vexc_debug("Guest arch: %d\n", guest);
    vexc_debug("Guest arch hwcaps: %08x\n", archinfo.hwcaps);
